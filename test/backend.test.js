...
const ctx = { SpreadsheetApp:{getActiveSpreadsheet:()=>ss}, LockService:{getScriptLock:()=>({waitLock(){},releaseLock(){}})},
  ContentService:{ createTextOutput:t=>({ t, setMimeType(){ return JSON.parse(t); } }), MimeType:{JSON:1} }, Logger:{log(){}}, Math, JSON, String, Array, Object, Date, Set, Error, RegExp };
vm.createContext(ctx); vm.runInContext(fs.readFileSync(__dirname + '/../apps-script/Code.gs', 'utf8'), ctx);
